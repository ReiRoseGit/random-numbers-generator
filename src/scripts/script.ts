...
const formElem: HTMLFormElement | null = document.querySelector("#formElem");
...
// Создание websocket
const ws: WebsocketGeneration = new WebsocketGeneration(
...
async function generateNumbers(e: SubmitEvent): Promise<void> {
    ...
    if (
        ...
        sortedData &&
        time &&
        Validation.cleanParams(bound, flows)
    ) {
        const checkBox: HTMLInputElement | null =
            document.querySelector("#exampleCheck1");
        if (checkBox && checkBox.checked) {
            // Ветка для динамического вывода чисел
            ws.printDynamicNumbers(
                bound.value,
                flows.value,
                unsortedData,
                sortedData,
